...
class Search extends Component {
   constructor(props, context){
      ...
      this.state = {
         active: null,
         term: context.store.search.term
      };
   }
   ...
   render() {
      ...
      const {data} = this.context.store.search;
      ...
      const _data = (data || []).map(item=>{
         ...
         return (
            <Link key={item.id} to={link} className="item">
               {!item.image ? null : <img src={item.image.replace("/image/upload/", "/image/upload/w_100,h_100,c_fill,q_100,e_vibrance:33/")} className="ui avatar image"/>}
