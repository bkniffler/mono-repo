var multi = require("./multi-bind");
var log = require('debug')("api", "autobinder");
...
var restEndpoints = function(model){
   ...
   return{
      ...
      getById: function (req, res, next) {
         ...
         if(id.length === 36){
            model.findById(id, req.query).then(function(data){
               if(!data){
            ...
            model.findAll(req.query).then(function(data){
      ...
      put: function (req, res, next) {
         var id = req.params.id;
         model.findById(id, req.query).then(function (item) {
            if (!item) {
               return next(new Error("Item with id " + id + " not found"));
            }
